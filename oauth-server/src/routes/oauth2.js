...
function issueTokens(userId, clientId, done) {
  db.users.findById(userId, (error, user) => {
    const accessToken = utils.getUid(255);
...
server.exchange(oauth2orize.exchange.code((client, code, redirectUri, done) => {
  db.authorizationCodes.find(code, (error, authCode) => {
    ...
    issueTokens(authCode.userId, authCode.clientId, done);
  });
}));
...
server.exchange(oauth2orize.exchange.password((client, username, password, scope, done) => {
  ...
  db.clients.findByClientId(client.clientId, (error, localClient) => {
    ...
    // Validate the user
    db.users.findByEmail(username, (error, user) => {
      if (error) { return done(error); }
      ...
      issueTokens(user.id, localClient.id, done);
    });
  });
}));
...
server.exchange(oauth2orize.exchange.clientCredentials((client, scope, done) => {
  ...
  db.clients.findByClientId(client.clientId, (error, localClient) => {
    ...
    issueTokens(null, client.id, done);
  });
}));
...
module.exports.authorization = [
  ...
  server.authorization((clientId, redirectUri, done) => {
    ...
    db.accessTokens.findByUserIdAndClientId(user.id, client.id, (error, token) => {
      // Auto-approve
